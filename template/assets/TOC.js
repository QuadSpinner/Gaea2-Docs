...
function initGlobalToc({
    ...
    // ---------- build: preserve order, render top-level "sections" as <h3> ----------
    let topUl = null;

    function ensureTopUl() {
        if (topUl) return topUl;
        topUl = document.createElement("ul");
        host.appendChild(topUl);
        return topUl;
    }
    ...
    for (const node of roots) {
        ...
        if (!isTopSection) {
            ensureTopUl().appendChild(renderNode(node));
            continue;
        }
    ...
    // ---------- initial collapsed state for ALL groups/subgroups ----------
    ...
    // ---------- node renderer (groups/subgroups remain collapsible) ----------
