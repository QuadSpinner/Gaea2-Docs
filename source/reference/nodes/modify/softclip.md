---
description: SoftClip a low-level utility that lets you softly reduce the intensity of shapes above a specified threshold. It can be comparable to masking a shape with a Height node and then clamping that portion.
title: SoftClip
uid: softclip
order: 31
---

# SoftClip

<!-- AUTO-VIDEO-LINK:START -->
## Official Video

For a video breakdown of this node, see [the official SoftClip video](/videos/official/nodes/yt-softclip.html).
<!-- AUTO-VIDEO-LINK:END -->

SoftClip a low-level utility that lets you softly reduce the intensity of shapes above a specified threshold. It can be comparable to masking a shape with a Height node and then clamping that portion. Unlike such a custom set up, the SoftClip node gives better results at a tremendously faster speed and requires no setup.

## When to Use It
Use SoftClip when peaks or high values need to be reduced gently instead of with a hard clamp.

## Visual Direction

![](/.data/assets/SoftClip_VD.png)

## Properties
